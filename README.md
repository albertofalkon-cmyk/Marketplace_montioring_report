...
Automated weekly **situation report** for Insurify's ad marketplace, produced separately for each vertical (**Auto** and **Home**). Each Monday it recomputes last week's numbers from the Marketplace Monitoring dashboard, an analyst step interprets them and selects the charts that best explain *that* week, and a polished PDF is generated and posted to Notion + Slack.
...
## Two layers
**1. The pipeline (standalone Python).** Captures the real dashboard charts and builds the PDF. Runs on its own — no Claude, no connectors needed.
