...
Runs every Tuesday morning. Produces **two** situation reports for **last week (window ending the most recent Monday)** — one for **Auto** and one for **Home** — posts both under a single weekly entry in the Notion "Marketplace Monitoring Reports" page, then sends a Slack DM to Alberto. 
...
- Notion page id: `3b8a8367-d4b2-8157-8a4a-e706bc22e9e3` ("Marketplace Monitoring — Weekly Reports (Draft)").

## Steps

**0. Compute the window end date** = most recent Monday relative to past Monday. Format `YYYY-MM-DD`. Call it `END`.
...
**3. Post to Notion** (uses the Notion connector). Upload BOTH PDFs, then append ONE weekly entry with an Auto and a Home subsection:
